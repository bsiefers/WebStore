...
namespace WebStore.Tests.UnitTests.Admin.Products
{
    ...
    public class UpdateProductTest
    {
        ...
        public void TestUpdateProduct()
        {
            ...
            var updateProduct = new UpdateProduct(mockContext.Object);
            //run valid request
            RunValidTest(updateProduct, request);

            //bad requests
            request.Id = -1;
            RunBadRequest(updateProduct, request, 404);
            request.Id = 1;

            request.Name = null;
            RunBadRequest(updateProduct, request, 400);
            request.Name = name;
        ...
        public void RunValidTest(UpdateProduct updateProduct, UpdateProduct.Request request)
        {
            var actual = updateProduct.Do(request).Result;
        ...
        public void RunBadRequest(UpdateProduct updateProduct, UpdateProduct.Request request, int status)
        {
            var actual = updateProduct.Do(request).Result;
            actual.Should().NotBeNull();
